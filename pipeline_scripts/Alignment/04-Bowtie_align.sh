...
#    SRA_ID="${SRA_ID%\*}" 
    ...
    if [ $SEQ_TYPE = "PE" ] ; then 
        ...
        bowtie2 -p 12 \
            -x $Bowtie_index \
            -U ${PATH_reads}${SRA_ID}*${extension} \
            | samtools view -bS - > ${SRA_ID}_${FileNamePrefix}_mapping.bam


        # samtools view -b -@ 20 -f 4 ${SRA_ID}_${FileNamePrefix}_mapping.bam \
        #     | bamToFastq -i - \
        #         -fq ${SRA_ID}_${FileNamePrefix}_unmapped.fastq
        
        # --- Extract unmapped reads ---
        samtools fastq -@ 12 -f 4 ${SRA_ID}_${FileNamePrefix}_mapping.bam > ${SRA_ID}_${FileNamePrefix}_unmapped.fastq

    elif [ $SEQ_TYPE = "rnaSpades" ] ; then 
        ...
        PATH_reads=${PATH_INPUT}${SRA_ID}/transcripts.rnaSpades.fasta

        bowtie2 -p 12 --no-mixed --no-discordant \
            -x $Bowtie_index \
            -f ${PATH_reads} | samtools view -bS - >  rnaSpades_contigs_${FileNamePrefix}_mapping.bam
                
        
        samtools fasta -f 4 rnaSpades_contigs_${FileNamePrefix}_mapping.bam > rnaSpades_unmapped.rnaSpades.fasta
