...
for srr in ${SRA_IDs[@]:$index_iter:$n_operation} ; do
    ...
    if [ "$contigs_type" = "trinity" ] ; then
        ...
        if [ "$use_unmapped_contigs" = "FALSE" ] ; then
            contig=${path_to_contigs}${sra_id}/"transcripts.rnaSpades.fasta"
        else 
            contig=${path_to_contigs}${sra_id}/Bowtie2_mapping_hg38/rnaSpades_unmapped.rnaSpades.fasta
        fi
    ...
    if [ "$restart_Blast" = "TRUE" ] ; then
        ...
        seqkit grep --pattern-file ${path_to_contigs}${sra_id}/diff_list.txt \
            $contig > ${path_to_contigs}${sra_id}/restarted.${contigs_type}.fasta
