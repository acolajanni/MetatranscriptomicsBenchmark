...
def get_query_result_from_Blast_clean(path_to_query, return_title=False):
    ...
    if(return_title):
        return(hit[["contig_ID","evalue","pident","qcov_hsp","align_len","subject_acc","subject_title","staxids"]])
    else:        
...
parser.add_argument(
    ...
    type=bool,
    default=False,
    help='Return query title in the output dataframe (default: False)')
...
return_titles=args.ReturnTitleQuery
...
if args.QueryType == "blast":
    ...
    query = get_query_result_from_Blast_clean(file_path,return_titles)
