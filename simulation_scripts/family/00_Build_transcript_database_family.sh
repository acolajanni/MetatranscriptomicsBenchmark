...
file=selected_genomes_family.tsv
...
mkdir -p ${PATH_DATABASE}transcript_database/family/${SLURM_ARRAY_TASK_ID}/
rm -r ${PATH_DATABASE}transcript_database/family/${SLURM_ARRAY_TASK_ID}/*

PATH_TRANSCRIPT_DATABASE=${PATH_DATABASE}transcript_database/family/${SLURM_ARRAY_TASK_ID}/
...
for i in ${!genomeID_array[@]} ; do 
    ...
    if [ ! -f "${PATH_TRANSCRIPT_DATABASE}/${family}_transcripts.fasta" ]; then
        touch "${PATH_TRANSCRIPT_DATABASE}/${family}_transcripts.fasta"
    fi
    ...
    seqkit replace -p '(.+)' -r taxid:${taxid}'_${1}' ${SEQUENCE_DATABASE}${genomeID}/*.fna.gz \
        | sed '/^>/s/^>\([^ ]*\) .*/>\1 /' \
        | sed  's/lcl|//g' \
        | sed 's/ *$//g' >> ${PATH_TRANSCRIPT_DATABASE}/${family}_transcripts.fasta
...
# seqkit replace --threads 10 -p '(.+)' -r taxid:9606'_${1}' /shared/projects/microbiome_translocation/database/Grch38p14/GCF_000001405.40_GRCh38.p14_cds_cleaned.fna.gz \
#     | sed '/^>/s/^>\([^ ]*\) .*/>\1 /' \
#     | sed  's/lcl|//g' \
...
# seqkit replace --threads 10 -p '(.+)' -r taxid:9606'_${1}' /shared/projects/microbiome_translocation/database/T2T_CHM13v2.0/GCF_009914755.1_T2T-CHM13v2.0_rna_from_genomic.fna.gz \
#     | sed '/^>/s/^>\([^ ]*\) .*/>\1 /' \
#     | sed  's/lcl|//g' \
#     | sed 's/ *$//g' \
