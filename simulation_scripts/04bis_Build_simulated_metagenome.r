...
} else{path_database_short=paste0(path_database_short,folder)}
...
transcripts_database[["Homo_sapiens_rna"]] = read.delim(paste0(path,'database_clean/transcript_database/Homo_sapiens_rna_boolaray.tsv'))
...
for (p in phylum) {
  ...
  if (all_transcripts == "TRUE") {
    df=df[df$boolarray == TRUE,]
    n_transcript=nrow(df)
    
    writeLines(df$TranscriptID, paste0(current_dir,p,"_",n_transcript,".txt") )
    writeLines(df$TranscriptID, paste0(current_dir,p,"_all.txt") )
