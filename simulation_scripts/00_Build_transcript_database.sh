...
if [[ "$taxlvl" == "genus" ]]; then
    phylum_array=$(awk -v FS='\t' '{print $21}' ${PATH_SELECTED_GENOMES}selected_genomes.tsv)

elif [[ "$taxlvl" == "phylum" ]]; then
    phylum_array=$(awk -v FS='\t' '{print $17}' ${PATH_SELECTED_GENOMES}selected_genomes.tsv)

else
  echo "Unknown taxonomic level: $taxlvl" >&2
  exit 1
fi
