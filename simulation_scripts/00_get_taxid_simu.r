...
bcereus = refseq_full[refseq_full$taxid == "1396",]


bcereus$ftp_path = sapply(bcereus$ftp_path,
                                            function(x){ paste0(str_replace(x,"https://","ftp://"),"/")   })

bcereus$genomeID = basename(bcereus$ftp_path)

write.table(bcereus, file = paste0(distant_path, "database_clean/refseq_bcereus.tsv"),
            sep = '\t', quote = FALSE, row.names = FALSE, col.names = FALSE)


ggvirus=c(2560650,"GCF_002621085.1",2560650,"Gorganvirus ifashan","-",
  "-","-","-","ftp://ftp.ncbi.nlm.nih.gov/genomes/all/GCF/002/621/085/GCF_002621085.1_ASM262108v1/","-",
  "virus","-","-","-","-",
  "Viruses","Uroviricota","Caudoviricetes","unclassified Caudoviricetes order","unclassified Caudoviricetes family",
  "Gorganvirus","Gorganvirus isfahan", "unclassified Gorganvirus isfahan subspecies/strain", "GCF_002621085.1_ASM262108v1")

gorganvirus <- as.data.frame(t(ggvirus), stringsAsFactors = FALSE)
write.table(gorganvirus, file = paste0(distant_path, "database_clean/refseq_gorganvirus.tsv"),
            sep = '\t', quote = FALSE, row.names = FALSE, col.names = FALSE)
