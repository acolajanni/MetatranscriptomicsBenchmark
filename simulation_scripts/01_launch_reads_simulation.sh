...
transcript_dir="${4:-/shared/projects/microbiome_translocation/data/Simulation/with_replacement/transcripts/}"
array="${5:-TRUE}"
...
Rscript --no-save --no-restore \
    ${PATH_MAIN}fastq_scripts/Simulation/01_Simulate_reads_v2.r \
        ${PATH_DATA} \
        $transcript_dir \
        ${n_transcript} \
        ${read_per_transcript} \
